...
const int get_index(const int* array, const int length, const int target) {
	...
	//   plot momentum
		...
		// 				// muon stuff
		// 				if(branch.counter[hit] == 5) {
		// 					// stuff for pre-degrader
		// 					const int index = get_index(muon_ids_pre_deg, n_seen_pre_deg, branch.id[hit]);
		// 					if (index == -1) {
		// 						// new muon
		// 						muon_ids_pre_deg[n_seen_pre_deg++] = branch.id[hit];
		// 						muon_mom_pre_deg[n_seen_pre_deg] = get_momentum(branch, hit);
		// 					}
		// 				} else if (branch.counter[hit] == 1 && 
		// 						   branch.first_step[hit]) {
		// 					// first step in scint 1 now a triggering muon
		// 					const int index = get_index(muon_ids_pre_deg, n_seen_pre_deg, branch.id[hit]);
		// 					
		// 					if (index != -1) {
		// 						// we've logged its momentum etc
		// 						muon_ids_scint1[n_seen_scint1++] = branch.id[hit];
		// 					} else {
		// 						// this branch shouldn't execute
		// 						cout << "We've not see this one "<< branch.id[hit] <<endl;
		...
		// 				}
		// 				
		// 			} else if (abs(branch.pdgid[hit]) == 11) {
		// 				// electron/positron stuff
		// 				if ((branch.counter[hit] == 1 || 
		// 					 branch.counter[hit] == 3) && 
		// 					 branch.first_step[hit]) {
		// 				  	// first step in scint 1 or 2	
		// 					const int e_index = get_index(electron_ids, n_seen_electrons, branch.id[hit]);
		// 					if (e_index == -1) {
		// 						// new electron!
		// 						electron_ids[n_seen_electrons++] = branch.id[hit]; // log it!
		// 						const int parent_index = get_index(muon_ids_pre_deg, n_seen_pre_deg, branch.parentid[hit]);
