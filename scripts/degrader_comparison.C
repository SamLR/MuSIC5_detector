...
void degrader_comparison(){
    ...
    const TString file_roots [] = {"run_Aluminium_0.2mm",
        "run_Aluminium_10mm",
        "run_Aluminium_1mm",
        "run_Aluminium_2mm",
        "run_Aluminium_5mm",
    ...
    cut_func_ptr cuts[] = {&stopped_muons, &not_stopped_muons};
    const unsigned int n_files = 10;
    TH1F* hists [n_files][2]; // one for stopped, one for not

    for(unsigned int file_no = 0; file_no < n_files; ++file_no) {
        TH1F** h_pair = hists[file_no]; // alias to the pair of hists we want
        ...
        h_pair[0] = new TH1F(name1, name1, 50, 0, 200); // fresh blood!
        h_pair[1] = new TH1F(name2, name2, 50, 0, 200);
        // loop-de-loop
        loop_entries<in_branch_struct, TH1F>(in_tree, branch, h_pair, 2, cuts, true);
        ...
        draw_pretty_two_hists(h_pair[0], h_pair[1], h_title, 
