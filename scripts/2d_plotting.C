...
void do_it_all2D(const unsigned int& n_files,
    ...
    TH2F* hists [n_files][n_funcs]; // one for stopped, one for not
    for(unsigned int file_no = 0; file_no < n_files; ++file_no) {
        in_branch_struct branch;
        TH2F** hist_set = hists[file_no]; // alias to the pair of hists we want
        ...
        // loop-de-loop
        loop_entries<in_branch_struct, TH2F>(in_tree, branch, hist_set, n_funcs, cuts, true);
        ...
        for(unsigned int i = 0; i < n_funcs; ++i) {
            ...
            draw_pretty_hist(hist_set[i], title, img_location);   
        }
    }
    out_file->Write();
...
void preDeg_vs_preST_muons(const in_branch_struct& branch, const TH2* hist, const bool verbose){
    ...
    for(unsigned int hit = 0; hit < n_hits; ++hit) {
        ...
        if (counter == 1002) {
            ...
            if (!seen_it) { 
                ...
                target_momentums.push_back(mom);
            } 
            ...
            if (!seen_it) { 
                ...
                for(unsigned int i = 0; i < target_tracks_ids.size(); ++i) {
                    if(target_tracks_ids[i] == trackID){
                        double mom = length (branch.px[hit], branch.py[hit], branch.pz[hit]);
                        hist->Fill(mom,target_momentums[i]);
