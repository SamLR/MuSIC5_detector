...
void tof_calculation(const int& n_files, // number of file roots
    ...
    const unsigned int n_funcs = 1;//2; // mainly a simple check for arrays
    ...
    const TString func_names[n_funcs] = {"pion and muon decay times"};

    // the functions applied to every hit
    // entry_fptr cuts[n_funcs] = {&muon_tof, &charged_tof};
    entry_fptr cuts[n_funcs] = {&charged_tof};
    ...
    for(unsigned int file = 0; file < n_files; ++file) {
        ...
        TString h_title2 = "Muon & pion decay times "+file_roots[file];
