...
:m_root(root),
m_g4bl_input_enabled(false), m_g4bl_charged_only(false), m_g4bl_pid_only(false),
                                            ...
                                            int &pdgid)
{
    ...
    } else if (m_g4bl_pid_only) { // only run the selected particle type
        const int pid = m_particleGun->GetParticleDefinition()->GetPDGEncoding();
        while (m_root->in_PDGid != pid) {
            m_root->g_iev += 1;
            m_root->tree_g4bl->GetEntry(m_root->g_iev);
        }
    }
